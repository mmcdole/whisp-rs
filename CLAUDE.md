...
```bash
cargo build                    # debug build (CUDA enabled by default)
cargo build --release          # release build
cargo build --no-default-features  # build without CUDA
...
whisp-rs is a Linux push-to-talk speech-to-text tool. It listens for a hotkey, captures audio, transcribes via whisper.cpp, and pastes the result into the active window.
...
3. **Transcriber thread** (`transcriber.rs`) — receives audio buffers, runs whisper.cpp beam search inference, sends text back
...
**Supporting modules:**
- `config.rs` — loads TOML config, resolves model paths (local file, HuggingFace Hub preset, or manual repo+file)
...
- **CUDA feature** enabled by default; `cmake/cuda17.cmake` sets C++17 for CUDA compatibility
...
- Model presets (tiny through distil-large-v3.5) auto-download from HuggingFace Hub via `hf-hub`
